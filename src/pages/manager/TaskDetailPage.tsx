...
export default function TaskDetailPage() {
  const { deptId, taskId } = useParams()
  const navigate = useNavigate()
  const analysis = MOCK_ANALYSES.find((a) => a.id === taskId) || MOCK_ANALYSES[0]
  ...
  return (
    <Shell breadcrumbs={[
      ...
      { label: '· Demo-to-Conversion', path: `/manager/dept/${deptId}/agent/wajeeha/sub/demo-conv` },
      { label: `Task ${analysis.demoId}` },
    ]}>
      <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-sm font-['DM_Sans'] text-[var(--text-muted)] hover:text-[var(--text-primary)] mb-6">
