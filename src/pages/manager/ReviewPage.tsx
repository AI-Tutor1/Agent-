...
import { MOCK_ANALYSES, type DemoAnalysis } from '@/data/mock'
...
export default function ReviewPage() {
  ...
  const [analyses, setAnalyses] = useState<DemoAnalysis[]>([...MOCK_ANALYSES])
            ...
            <div className="space-y-4">
              {filtered.map((a, i) => (
                <DemoAnalysisCard
                  ...
                  onExpand={(id) => navigate(`/manager/dept/product/agent/wajeeha/sub/demo-conv/task/${id}`)}
                />
              ))}
            </div>
          )}
