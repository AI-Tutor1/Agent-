...
export default function DeptPage() {
  ...
  useEffect(() => {
    api.departments.getById(params.deptId).then((d: Department) => {
      setDept(d)
      setLoading(false)
    }).catch(() => setLoading(false))
  }, [params.deptId])

  if (loading) return <Shell><p className="text-[var(--text-muted)]">Loading...</p></Shell>
        ...
        {dept.agents.map((agent, i) => {
          ...
          return (
            <motion.div
              ...
              onClick={() => isWajeeha ? router.push(`/manager/dept/${params.deptId}/agent/wajeeha`) : null}
